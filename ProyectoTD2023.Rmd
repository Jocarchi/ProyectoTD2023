...
# Se le asigna un valor (1000) a Frec
Frec <- 1000
# Se genera un vector de tiempo con la secuencia de 0 a la longitud del archivo dividido por la 
# frecuencia de muestreo y luego se divide en incrementos de 1/Frec.
tiempo_ana1 <- seq(from = 0, to = (length(fichero_ana1)-1)/Frec, by = 1/Frec)
# Se crea el data frame 
dato_ana1 <- data.frame(tiempo_ana1, fichero_ana1)
...
# filas del data frame "dato_dig1" con un intervalo de 0.25.
tiempo <- seq(0, length.out = nrow(dato_dig1), by = 0.25)
# Toma las primeras 4372 filas del vector "tiempo" y las asigna a la variable "tiempo".
...
dato_dig1
# Observamos en la tabla la información de diversas variables, las cuales corresponden a la frecuencia 
# cardiaca, (HR1 y HR2),medida por latidos por minuto, la saturación en sangre de oxígenos(MspO2),medida en # porcentaje, la presión diástola y sistolica (Psist,Pdia),  la VCP se mide en centímetros por segundo 
# (cm/s) y el tiempo, el cual se mide en segundos
```
...
# Se extrae el tamaño del archivo de registro digital.
max_fichdig1 <- fich_info$size
# Se lee el contenido del archivo y se guarda en un vector llamado "fichero_dig1" utilizando 
# la función "readBin()".
fichero_dig1 <- readBin(fich_dig1,integer(),n = max_fichdig1,size = 1, signed = F)
# Se divide el vector "fichero_dig1" en vectores separados para cada 
# parámetro de interés.
...
# Observamos en la tabla la información de diversas variables, las cuales corresponden a la frecuencia 
# cardiaca, (HR1 y HR2),medida por latidos por minuto, la saturación en sangre de oxígenos(MspO2),
# medida en porcentaje, la presión diástola y sistolica (Psist,Pdia),  la VCP se mide en
# centímetros por segundo (cm/s) y el tiempo, el cual se mide en segundos
```
...
# filas del data frame "dato_dig1" con un intervalo de 0.25.
tiempo <- seq(0, length.out = nrow(dato_dig1), by = 0.25)
# Toma las primeras 3540 filas del vector "tiempo" y las asigna a la variable "tiempo".
tiempo <- head(tiempo, 3540)
